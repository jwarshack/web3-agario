...
let tree = quadtree(0, 0, c.gameWidth, c.gameHeight);
...
let initMassLog = util.log(c.defaultPlayerMass, c.slowBase);


function addFood(toAdd) {
    let radius = util.massToRadius(c.foodMass)

    while(toAdd--) {
        let position = c.foodUniformDisposition ? util.uniformPosition(food, radius) : util.randomPosition(radius);
        food.push({
            // Make IDs unique.
...
function addVirus(toAdd) {
    while (toAdd--) {
        let mass = util.randomInRange(c.virus.defaultMass.from, c.virus.defaultMass.to, true);
        let radius = util.massToRadius(mass);
        let position = c.virusUniformDisposition ? util.uniformPosition(virus, radius) : util.randomPosition(radius);
        virus.push({
    ...
    for(let i=0; i<player.cells.length; i++)
    {
        ...
        for(let j=0; j<player.cells.length; j++) {
            if(j != i && player.cells[i] !== undefined) {
                let distance = Math.sqrt(Math.pow(player.cells[j].y-player.cells[i].y,2) + Math.pow(player.cells[j].x-player.cells[i].x,2));
                let radiusTotal = (player.cells[i].radius + player.cells[j].radius);
        ...
        if(player.cells.length > i) {
            let borderCalc = player.cells[i].radius / 3;
...
function balanceMass() {
    let totalMass = food.length * c.foodMass +
        users
            .map(function(u) {return u.massTotal; })
            .reduce(function(pu,cu) { return pu+cu;}, 0);

    let massDiff = c.gameMass - totalMass;
    let maxFoodDiff = c.maxFood - food.length;
    let foodDiff = parseInt(massDiff / c.foodMass) - maxFoodDiff;
    let foodToAdd = Math.min(foodDiff, maxFoodDiff);
    let foodToRemove = -Math.max(foodDiff, maxFoodDiff);
    ...
    let virusToAdd = c.maxVirus - virus.length;
